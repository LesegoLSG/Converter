...
const VoiceToTextPage = () => {
  ...
  const handleCopy = () => {
    navigator.clipboard
      ...
      .catch((err) => {
        alert("Failed to copy text to the clipboard");
      });
  };
  ...
  return (
    <div className="w-full min-h-screen flex flex-col justify-center items-center bg-gradient-to-r from-darkColor to-secondary text-white p-4">
      ...
      <div className="w-full max-w-4xl flex flex-col justify-center items-center gap-y-4">
        ...
        {/* Copy button */}
        <button
          className="bg-blue-600 text-white rounded-lg px-6 py-3 hover:scale-105 transform transition-transform"
          onClick={handleCopy}
        >
          Copy to Clipboard
        </button>
